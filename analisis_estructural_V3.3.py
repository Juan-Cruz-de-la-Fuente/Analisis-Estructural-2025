...
def formatear_unidades(valor, tipo="presion"):
    ...
    if tipo == "presion":
        ...
        if abs_valor == 0:
            ...
            return f"{valor*1e9:.3f} nN/mm"
        elif abs_valor < 1e-3:
            return f"{valor*1e6:.3f} μN/mm"
        elif abs_valor < 1:
            return f"{valor*1e3:.3f} mN/mm"
        elif abs_valor < 1e3:
            return f"{valor:.3f} N/mm"
        elif abs_valor < 1e6:
            return f"{valor/1e3:.3f} kN/mm"
        elif abs_valor < 1e9:
            return f"{valor/1e6:.3f} MN/mm"
        else:
            return f"{valor/1e9:.3f} GN/mm"
...
def visualizar_estructura_moderna(mostrar_deformada=False, factor_escala=10):
    """Visualizar la estructura con estilo moderno minimalista (VERSIÓN CON ETIQUETAS INTELIGENTES)"""
    ...
    # --- Dibujo de estructura original punteada (sin cambios) ---
    ...
    # --- Dibujo de elementos principales y sus etiquetas ---
    for elemento in st.session_state.elementos:
        nodo_inicio = next((n for n in st.session_state.nodos if n['id'] == elemento['nodo_inicio']), None)
        nodo_fin = next((n for n in st.session_state.nodos if n['id'] == elemento['nodo_fin']), None)
        ...
        if nodo_inicio and nodo_fin:
            if mostrar_deformada and nodos_deformados:
                # Lógica para la estructura deformada
                ...
                if nodo_inicio_def and nodo_fin_def:
                    ...
                    p_inicio_ref = nodo_inicio_def
                    p_fin_ref = nodo_fin_def
            else:
                # Lógica para la estructura original
                ax.plot([nodo_inicio['x'], nodo_fin['x']], 
                       [nodo_inicio['y'], nodo_fin['y']], 
                       color='#000000', linewidth=3, alpha=0.9)
                p_inicio_ref = nodo_inicio
                p_fin_ref = nodo_fin

            # --- INICIO DE LA NUEVA LÓGICA DE POSICIONAMIENTO DE ETIQUETAS ---
            mid_x = (p_inicio_ref['x'] + p_fin_ref['x']) / 2
            mid_y = (p_inicio_ref['y'] + p_fin_ref['y']) / 2

            dx = p_fin_ref['x'] - p_inicio_ref['x']
            ...
            # Solo aplicar offset si la barra no es un punto
            if longitud > 1e-6:
                # Vector perpendicular a la barra: (-dy, dx)
                # Normalizamos este vector dividiendo por la longitud
                offset_x = -dy / longitud * distancia_offset
                offset_y = dx / longitud * distancia_offset
            
            # Dibujar la etiqueta con el offset calculado
            ax.text(mid_x + offset_x, mid_y + offset_y, f'E{elemento["id"]}', 
    ...
    for i, nodo in enumerate(st.session_state.nodos):
        if nodo['tipo'] == 'fijo':
            ...
            color = '#2563EB'; edge_color = '#1D4ED8'
        
        circle = plt.Circle((nodo['x'], nodo['y']), 0.03, color=color, alpha=0.9, zorder=10)
        ax.add_patch(circle)
        circle_edge = plt.Circle((nodo['x'], nodo['y']), 0.03, fill=False, edgecolor=edge_color, linewidth=1, zorder=11)
        ax.add_patch(circle_edge)
        ax.text(nodo['x'], nodo['y'], str(nodo['id']), ha='center', va='center', fontsize=6, fontweight='700', color='white', zorder=12, fontfamily='sans-serif')
        ax.text(nodo['x'], nodo['y'] - 0.15, f"({nodo['x']:.1f}, {nodo['y']:.1f})", ha='center', va='top', fontsize=5, color='#374151', fontfamily='sans-serif', fontweight='500')

        if mostrar_deformada and nodos_deformados:
            nodo_def = nodos_deformados[i]
            circle_def = plt.Circle((nodo_def['x'], nodo_def['y']), 0.025, color='#28a745', alpha=0.8, zorder=10)
            ax.add_patch(circle_def)
            circle_def_edge = plt.Circle((nodo_def['x'], nodo_def['y']), 0.025, fill=False, edgecolor='#218838', linewidth=1, zorder=11)
            ax.add_patch(circle_def_edge)
            ax.plot([nodo['x'], nodo_def['x']], [nodo['y'], nodo_def['y']], color='#6B7280', linestyle=':', linewidth=1.5, alpha=0.7)
    ...
    if mostrar_deformada:
        ax.set_title(f'Estructura Deformada (Factor: {factor_escala}×)', fontsize=16, fontweight='700', fontfamily='sans-serif', color='#111827', pad=20)
    else:
        ax.set_title('Estructura Original', fontsize=16, fontweight='700', fontfamily='sans-serif', color='#111827', pad=20)
    ...
    if mostrar_deformada:
        legend_elements = [
            plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='#DC2626', markersize=8, label='Nodos Fijos', markeredgecolor='#991B1B'),
            plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='#2563EB', markersize=8, label='Nodos Libres', markeredgecolor='#1D4ED8'),
            plt.Line2D([0], [0], color='#9CA3AF', linewidth=2, linestyle='--', label='Estructura Original'),
            plt.Line2D([0], [0], color='#000000', linewidth=3, label='Estructura Deformada'),
            plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='#28a745', markersize=6, label='Nodos Deformados', markeredgecolor='#218838')
        ]
    else:
        legend_elements = [
            plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='#DC2626', markersize=8, label='Nodos Fijos', markeredgecolor='#991B1B'),
    ...
    ax.legend(handles=legend_elements, loc='upper right', frameon=True, fancybox=True, shadow=True, fontsize=10, prop={'family': 'sans-serif', 'weight': '500'})
        ...
        for fila in matriz:
            ...
            for valor in fila:
                if "rigidez" in titulo.lower() or "K" in titulo:
                    fila_formateada.append(formatear_unidades(valor, "rigidez"))
                else:
...
def generar_excel_completo():
    ...
    try:
        ...
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            ...
            pd.DataFrame(info_data).to_excel(writer, sheet_name='Información', index=False)
            
            # Hoja 2: Nodos
            crear_tabla_nodos().to_excel(writer, sheet_name='Nodos', index=False)
            
            # Hoja 3: Conectividad
            crear_tabla_conectividad().to_excel(writer, sheet_name='Conectividad', index=False)
            
            # Hoja 4: Matriz de Rigidez
            resultado = st.session_state.resultados
            K_data = []
            for i, fila in enumerate(resultado['K_global']):
                fila_dict = {f'GL{j+1}': formatear_unidades(val, "presion") for j, val in enumerate(fila)}; fila_dict['GL'] = f'GL{i+1}'; K_data.append(fila_dict)
            df_K = pd.DataFrame(K_data); cols = ['GL'] + [f'GL{i+1}' for i in range(len(resultado['K_global']))]; df_K = df_K[cols]
            df_K.to_excel(writer, sheet_name='Matriz_Rigidez', index=False)
            
            # Hoja 5: Resultados Globales (Fuerzas y Desplazamientos)
            resultados_data = []
            for i, (info, fuerza, desplazamiento) in enumerate(zip(st.session_state.grados_libertad_info, resultado['fuerzas'], resultado['desplazamientos'])):
                ...
                resultados_data.append({
                    ...
                    'Nombre_Fuerza': nombre, 'Fuerza': formatear_unidades(fuerza, "fuerza"),
                    'Detalle Fuerza': detalle_fuerza, 'Desplazamiento': formatear_unidades(desplazamiento, "desplazamiento"),
                    'Detalle Desplazamiento': detalle_desplazamiento, 'Fuerza_Valor_Numerico': fuerza,
...
def generar_pdf_completo(factor_escala=10):
    ...
    try:
        ...
        if st.session_state.nodos:
            ...
            for nodo in st.session_state.nodos:
                nodos_data.append([str(nodo['id']), nodo['tipo'].title(), f"{nodo['x']:.3f}", f"{nodo['y']:.3f}", str(nodo['grados_libertad_globales'][0]), str(nodo['grados_libertad_globales'][1])])
            nodos_table = Table(nodos_data)
            nodos_table.setStyle(TableStyle([('BACKGROUND', (0, 0), (-1, 0), colors.grey), ('TEXTCOLOR', (0, 0), (-1, 0), colors.whitesmoke), ('ALIGN', (0, 0), (-1, -1), 'CENTER'), ('FONTNAME', (0, 0), (-1, 0), 'Helvetica-Bold'), ('FONTSIZE', (0, 0), (-1, 0), 12), ('BOTTOMPADDING', (0, 0), (-1, 0), 12), ('BACKGROUND', (0, 1), (-1, -1), colors.beige), ('GRID', (0, 0), (-1, -1), 1, colors.black)]))
        ...
        if st.session_state.nodos:
            ...
            if fig_original:
                img_buffer_original = io.BytesIO(); fig_original.savefig(img_buffer_original, format='png', dpi=300, bbox_inches='tight'); img_buffer_original.seek(0)
                story.append(Image(img_buffer_original, width=400, height=300)); plt.close(fig_original); story.append(Spacer(1, 20))
        ...
        # Nuevo encabezado de la tabla
        resultados_data = [['GL', 'Nodo', 'Dir', 'Fuerza', 'Detalle', 'Desplaz.', 'Detalle']]
        for i, (info, fuerza, desplazamiento) in enumerate(zip(
            st.session_state.grados_libertad_info, 
            resultado['fuerzas'], 
            resultado['desplazamientos']
        )):
            detalle_fuerza = "Dato" if info['fuerza_conocida'] else "Incógnita"
            detalle_desplazamiento = "Dato" if info['desplazamiento_conocido'] else "Incógnita"
            ...
            resultados_data.append([
                ...
                detalle_fuerza, # Nueva columna
                formatear_unidades(desplazamiento, "desplazamiento"),
                detalle_desplazamiento # Nueva columna
            ])
        
        # Ajustar anchos de columna para las nuevas columnas
        ...
        datos_tensiones = calcular_datos_tensiones(resultado)
        if datos_tensiones:
            tensiones_pdf_data = [['Elemento', 'Material', 'Deformación (ε)', 'Tensión (σ)']]
            for item in datos_tensiones:
                tensiones_pdf_data.append([str(item['Elemento']), item['Material'], item['Deformación Unitaria (ε)'], item['Tensión (σ)']])
            tensiones_table = Table(tensiones_pdf_data, colWidths=[_ * inch for _ in [0.8, 1.8, 1.5, 1.5]]); tensiones_table.setStyle(TableStyle([('BACKGROUND', (0, 0), (-1, 0), colors.grey), ('TEXTCOLOR', (0, 0), (-1, 0), colors.whitesmoke), ('ALIGN', (0, 0), (-1, -1), 'CENTER'), ('FONTNAME', (0, 0), (-1, 0), 'Helvetica-Bold'), ('FONTSIZE', (0, 0), (-1, -1), 10), ('BOTTOMPADDING', (0, 0), (-1, 0), 12), ('BACKGROUND', (0, 1), (-1, -1), colors.beige), ('GRID', (0, 0), (-1, -1), 1, colors.black)]))
            story.append(tensiones_table)
        ...
        if st.session_state.elementos:
            ...
            for elem in st.session_state.elementos:
                conectividad_data.append([str(elem['id']), elem['tipo'], str(elem['nodo_inicio']), str(elem['nodo_fin']), elem['material'][:15] + '...' if len(elem['material']) > 15 else elem['material'], f"{elem['area']:.6f}", f"{elem['longitud']:.3f}", f"{elem['beta']:.4f}"])
            conectividad_table = Table(conectividad_data); conectividad_table.setStyle(TableStyle([('BACKGROUND', (0, 0), (-1, 0), colors.grey), ('TEXTCOLOR', (0, 0), (-1, 0), colors.whitesmoke), ('ALIGN', (0, 0), (-1, -1), 'CENTER'), ('FONTNAME', (0, 0), (-1, 0), 'Helvetica-Bold'), ('FONTSIZE', (0, 0), (-1, 0), 10), ('BOTTOMPADDING', (0, 0), (-1, 0), 12), ('BACKGROUND', (0, 1), (-1, -1), colors.beige), ('GRID', (0, 0), (-1, -1), 1, colors.black), ('FONTSIZE', (0, 1), (-1, -1), 8)]))
            story.append(conectividad_table); story.append(Spacer(1, 20))
        ...
        for i, fila in enumerate(resultado['K_global']):
            ...
            for val in fila: fila_formateada.append(f"{val:.2e}")
            K_data.append(fila_formateada)
        K_table = Table(K_data); K_table.setStyle(TableStyle([('BACKGROUND', (0, 0), (-1, 0), colors.grey), ('BACKGROUND', (0, 0), (0, -1), colors.grey), ('TEXTCOLOR', (0, 0), (-1, 0), colors.whitesmoke), ('TEXTCOLOR', (0, 0), (0, -1), colors.whitesmoke), ('ALIGN', (0, 0), (-1, -1), 'CENTER'), ('FONTNAME', (0, 0), (-1, 0), 'Helvetica-Bold'), ('FONTNAME', (0, 0), (0, -1), 'Helvetica-Bold'), ('FONTSIZE', (0, 0), (-1, -1), 6), ('BOTTOMPADDING', (0, 0), (-1, 0), 8), ('BACKGROUND', (1, 1), (-1, -1), colors.beige), ('GRID', (0, 0), (-1, -1), 1, colors.black)]))
...
def calcular_datos_tensiones(resultado):
    ...
    for elemento in st.session_state.elementos:
        if 'grados_libertad_global' not in elemento or 'material' not in elemento:
            continue # Omitir elementos mal configurados
    ...
    return tensiones_data

def mostrar_tabla_tensiones(datos_tensiones):
    """
    Toma los datos de tensiones y los muestra en una tabla de Streamlit.
    """
    ...
    if datos_tensiones:
        ...
        st.warning("No se pudieron calcular los datos de tensiones.")
...
# ------------ BLOQUE DEL PASO 8  -----------------
    ...
    if st.session_state.resultados:
        ...
        for i, (info, fuerza, desplazamiento) in enumerate(zip(
            ...
            nombre = st.session_state.nombres_fuerzas.get(i+1, f"F{i+1}")
            # Se añaden textos más descriptivos
            detalle_fuerza = "Dato (Entrada)" if info['fuerza_conocida'] else "Incógnita (Calculado)"
            detalle_desplazamiento = "Dato (Entrada)" if info['desplazamiento_conocido'] else "Incógnita (Calculado)"
            ...
            resultados_data.append({
                ...
                'Detalle Fuerza': detalle_fuerza,  # Nueva columna
                'Desplazamiento': formatear_unidades(desplazamiento, "desplazamiento"),
                'Detalle Desplazamiento': detalle_desplazamiento # Nueva columna
            })
        ...
        datos_tensiones = calcular_datos_tensiones(resultado)
        mostrar_tabla_tensiones(datos_tensiones)
        ...
        with col1:
            ...
            if pdf_data:
                st.download_button(label="📄 Descargar Reporte PDF", data=pdf_data, file_name=f"analisis_estructural_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf", mime="application/pdf", type="primary", use_container_width=True)
        with col2:
            st.markdown("##### Opciones para Excel")
            st.write(" ")
            excel_data = generar_excel_completo()
            if excel_data:
                st.download_button(label="📊 Descargar Datos Excel", data=excel_data, file_name=f"datos_analisis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", type="primary", use_container_width=True)
else:
