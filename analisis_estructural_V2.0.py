    ...
    for elemento in st.session_state.elementos_interactivos:
        ...
        if nodo_inicio and nodo_fin:
            fig.add_trace(go.Scatter(
                ...
                text=f"Barra {elemento['id']}: {nodo_inicio['id']} → {nodo_fin['id']}",
                hoverinfo='text'
            ))
...
def transferir_datos_interactivos():
    ...
    # Avanzar al paso de configuración de elementos
...
if st.session_state.step == 0:
    ...
    # Mostrar gráfico y capturar eventos
    clicked = st.plotly_chart(fig, use_container_width=True, return_events=True)
    
    # Procesar eventos de clic
    if clicked and "click" in clicked:
        click_data = clicked["click"]
        if "points" in click_data and len(click_data["points"]) > 0:
            # Clic en un nodo existente
            ...
            if "curveNumber" in point:
                ...
                if "pointIndex" in point:
                    ...
                    # Determinar el ID del nodo según la curva y el índice
                    if curve_num == 0:  # Nodos fijos
                        nodos_fijos = [n for n in st.session_state.nodos_interactivos if n['tipo'] == 'fijo']
                        if point_idx < len(nodos_fijos):
                            nodo_id = nodos_fijos[point_idx]['id']
                    else:  # Nodos libres
                        nodos_libres = [n for n in st.session_state.nodos_interactivos if n['tipo'] == 'libre']
                        if point_idx < len(nodos_libres):
                            nodo_id = nodos_libres[point_idx]['id']
                    
                    # Gestionar selección para crear barras
                    if st.session_state.nodo_seleccionado_interactivo is None:
                        st.session_state.nodo_seleccionado_interactivo = nodo_id
                        st.info(f"Nodo {nodo_id} seleccionado. Haga clic en otro nodo para crear una barra.")
                    else:
                        nodo_inicio = st.session_state.nodo_seleccionado_interactivo
                        nodo_fin = nodo_id
                        
                        if nodo_inicio != nodo_fin:
                            elemento_id = agregar_elemento_interactivo(nodo_inicio, nodo_fin)
                            if elemento_id:
                                st.success(f"Barra {elemento_id} creada entre nodos {nodo_inicio} y {nodo_fin}")
                        else:
            ...
            # Clic en un espacio vacío - añadir nuevo nodo
            x = click_data["points"][0]["x"]
            y = click_data["points"][0]["y"]
            
            nodo_id = agregar_nodo_interactivo(x, y, tipo_nodo)
            st.success(f"Nodo {nodo_id} ({tipo_nodo}) añadido en ({x:.2f}, {y:.2f})")
            st.session_state.nodo_seleccionado_interactivo = None
            st.rerun()
    ...
    if st.session_state.modo == "interactivo":
        # Para modo interactivo, usar elementos_interactivos como base
        elementos_base = st.session_state.elementos_interactivos
    else:
        # Para modo manual, usar el número especificado
    ...
    st.header("Paso 7: Configuración de Incógnitas")
    st.write("Configure las fuerzas y desplazamientos conocidos/desconocidos")
    
    # Configuración automática
    col1, col2 = st.columns(2)
    with col1:
        auto_calcular_toggle = st.checkbox("Cálculo Automático", value=st.session_state.auto_calcular)
        st.session_state.auto_calcular = auto_calcular_toggle
    
    with col2:
        if st.button("🔄 Recalcular Ahora", type="secondary"):
            auto_calcular()
    
    # Mostrar información de grados de libertad
    st.subheader("📊 Configuración de Grados de Libertad")
    
    with st.form("configuracion_gl"):
        for i, info in enumerate(st.session_state.grados_libertad_info):
            gl_num = info['numero']
            
            st.write(f"**GL{gl_num}** - Nodo {info['nodo']} ({info['direccion']})")
            
            col1, col2, col3, col4 = st.columns(4)
            
            with col1:
                desp_conocido = st.checkbox(f"Desplazamiento Conocido", 
                                          value=info['desplazamiento_conocido'],
                                          key=f"desp_conocido_{gl_num}")
            
            with col2:
                if desp_conocido:
                    valor_desp = st.number_input(f"Valor (m)", 
                                               value=info['valor_desplazamiento'] or 0.0,
                                               format="%.6f",
                                               key=f"valor_desp_{gl_num}")
                else:
                    valor_desp = None
                    st.write("Desconocido")
            
            with col3:
                fuerza_conocida = st.checkbox(f"Fuerza Conocida", 
                                            value=info['fuerza_conocida'],
                                            key=f"fuerza_conocida_{gl_num}")
            
            with col4:
                if fuerza_conocida:
                    valor_fuerza = st.number_input(f"Valor (N)", 
                                                 value=info['valor_fuerza'] or 0.0,
                                                 format="%.3f",
                                                 key=f"valor_fuerza_{gl_num}")
                    
                    # Campo para nombre personalizado
                    nombre_actual = st.session_state.nombres_fuerzas.get(gl_num, f"F{gl_num}")
                    nombre_fuerza = st.text_input(f"Nombre", value=nombre_actual, key=f"nombre_{gl_num}")
                    st.session_state.nombres_fuerzas[gl_num] = nombre_fuerza
                else:
                    valor_fuerza = None
                    st.write("Desconocida")
            
            # Actualizar información
            st.session_state.grados_libertad_info[i].update({
                'desplazamiento_conocido': desp_conocido,
                'valor_desplazamiento': valor_desp,
                'fuerza_conocida': fuerza_conocida,
                'valor_fuerza': valor_fuerza
            })
    ...
    # Mostrar matrices de elementos
    if st.session_state.matrices_elementos:
        st.subheader("🔧 Matrices de Rigidez de Elementos")
        ...
        for elemento_id, matrices in st.session_state.matrices_elementos.items():
            ...
            if elemento:
                with st.expander(f"Elemento {elemento_id} - {elemento['tipo']} (Nodos {elemento['nodo_inicio']} → {elemento['nodo_fin']})"):
                    
                    col1, col2 = st.columns(2)
                    
                    with col1:
                        mostrar_matriz_formateada(matrices['simbolica'], 
                                                f"Matriz Simbólica K{elemento_id}", 
                                                es_simbolica=True)
                    
                    with col2:
                        mostrar_matriz_formateada(matrices['numerica'], 
                                                f"Matriz Numérica K{elemento_id}", 
                                                es_simbolica=False)
                    
                    # Información del elemento
                    st.write(f"**Material:** {elemento['material']}")
                    st.write(f"**Área:** {elemento['area']:.6f} m²")
                    st.write(f"**Longitud:** {elemento['longitud']:.3f} m")
                    st.write(f"**Ángulo β:** {math.degrees(elemento['beta']):.2f}°")
    ...
    if st.session_state.resultados and st.session_state.resultados['exito']:
        ...
        **¡Su opinión es importante!** 
        ...
        [**📋 Evaluar Sistema de Análisis Estructural**](https://forms.gle/ejemplo)
